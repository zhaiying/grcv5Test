...
class Project (object):
    ...
    def __init__(self):
        self.cf_file = os.path.join (os.path.dirname(__file__), 'project.ini')
        self.read ()
    ...
    def get(self, cf, section, key, default_value='', value_type=str, force=False):
        ...
        try:
            ...
            if (force):
                raise Exception (
                    'can not get the configuration(section=%s,key=%s) from ini(%s)' % (section, key, self.cf_file))
            else:
                v = default_value
        try:
            ...
            raise Exception ('can not get proper type of the configuration(section=%s,key=%s) from ini(%s)' % (
            section, key, self.cf_file))
        return v
    ...
    def runtests(self):
        ...
        tests = self.testsuit ()
        fp = open (filename, 'wb')
        # runner = SmartHTMLTestRunner (stream=fp, title=self.report_title, description=self.report_description,
        #                               project=self)
       # runner.run (tests)
        fp.close ()
